...
\textcolor{red}{Ask a}\\
\response{We varied it randomly. Although the variation was identical between both the environments. The variation was introduced by the number of threads as mentioned in section 3.3 \textit{"The workload variation was introduced by..."}. }
...
\response{The use cases are predefined in the performance testing suite which is a limitation of our subject systems. We now discuss it in our revised threads to validity.}
...
\response{We chose linear regression model as it is used in prior work \cite{xiong2013vperfguard}\cite{Shang:2015:ADP:2668930.2688052}. Also, the model is straightfoward to explain compared to that other models that we may have used. We have added this brief in our revised manuscript. }
...
\textcolor{red}{Nistor ICSE 13\cite{Nistor}, Jin PLDI 12\cite{Jin} + 2/3}\\\\
\response{}
...
\response{We agree. We have now made the data public.} \textcolor{red}{should i mention the url?}
...
\response{Thank you for your feedback. We have toned it down in the revised manuscript.}\\\textcolor{red}{I am not sure what is he trying to say here}
...
\response{We do not compare the two environments based on load tests. We do it on the basis of typical analysis of a performance test. \cite{jack_tse}\cite{nguyen2012automated}}
...
\response{We restore the environments before restarting tests such as the database server. \textcolor{red}{he specifically says IOPS, maybe we can mention we  emptied the caches or restarted the system?}
	}
...
\response{Please refer to our response to the comment R2.30. We also mention in Section 7 on how we have contributed based on this work. We revise and update the conclusions as follows:
	\begin{itemize}
		\item The performance test from virtual environments can not be reused so the results can not be transferred as-is across environments especially with the \textit{MAPE} values as big as 600\%.
		\item We also evaluate the method proposed by \textit{Nguyen et al.} to use testing results in a different environment. Although, in their case, the environment are not virtual and physical.
		\item We propose a way that by \textit{normalization by deviance}, practitioners may reduce the discrepancy present.		
	\end{itemize}
