...
\response{We used such an approach to remove multicollinearity that is present between performance metrics. In fact, the approach is based on a popular statistical analysis, [Kuhn, Building Predictive Models in R Using the caret Package, Journal of Statistical Software] \cite{cor_R}. In essence, whenever two metrics are found to be highly correlated, one of them needs to be removed. To determine which one to remove, the approach chooses the one that is more highly correlated with the rest of the metrics. We elaborate the description of our approach in the new revision of the paper.
	}
