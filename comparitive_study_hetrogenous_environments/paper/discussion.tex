...
\begin{table}[tbh]
	\centering
	\caption{Median absolute percentage error from building a model using physical environment and testing on VMWare data.}
	\label{tab:vmware}
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{|c||c|c|}
			\hline
			...
			& \textbf{CloudStore} & \textbf{DS2} \\ %\hline
			\midrule
			External validation with normalization by deviance & 0.12 &0.19 \\ \hline
			External validation with normalization by load & 4.79& 1.62 \\ \hline
		\end{tabular}%
	}
\end{table}
