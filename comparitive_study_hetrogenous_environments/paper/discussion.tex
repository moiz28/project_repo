\emad{Overall, this section seems rushed to me, but again we do not have much space here.}
...
\emad{The purpose of this subsection in not clear to me}
...
We study the impact of changing the allocated resources on the results of our analysis. We change the computing resources allocated to the virtual environments by increasing the CPU to be 3 cores and increasing the memory to be 5GB \emad{why do we compare to this setup?}. Similarly, we train statistical models on the new performance testing results and tested it on the performance testing results from the physical environment. Similar to the results shown in Table~\ref{tab:errors}, the prediction error is high when normalizing based on load (1.57 for DS2 and 1.25 for CloudStore), while normalizing based on deviance can significantly reduce the error (0.09 for DS2 and 0.07 for CloudStore). Such results demonstrate the ability of reducing discrepancy in performance testing results by using normalization based on deviance. \emad{Is this what we want to say or that our results are the same as the earlier setup.}
