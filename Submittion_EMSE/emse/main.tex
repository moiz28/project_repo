...
There has been much work on performance testing and bug detection. Nistor \textit{et al$.$} detect the presence of functional and loop-related performance bugs with the help of their developed tool \cite{Nistor}. Jin \textit{et al$.$} present a study on wide range of performance bugs. \cite{Jin}. The authors examine rule-based performance bug detection in performance bug patches . Nistor \textit{et al$.$} in another study highlight that performance bug detection based on tools is limited \cite{nistor_2}. The authors also comment that performance bugs are mostly detected by code reasoning rather than seeing the effects of the system by the end user . Tsakiltsidis {et al$.$} use prediction models to detect and predict performance bugs based on extraction from source code repositories \cite{Tsakiltsidis}. Malik {et al$.$} present a study to uncover functional bugs via load testing\cite{h_malik_p_bugs}. The authors propose an approach to reduce the large amount of performance metrics at the end of a load test by principal component analysis. Zaman {et al$.$} study the tracking and fixing of performance bugs\cite{zaman_p_bugs}.

However, none of the above mentioned performance bug detection approach has been applied to different environment. In most of the cases the environment is not explicitly mentioned. Hence, to generalize the findings across environments remains an open topic.
...
The nature of our performance tests was based on our related studies mentioned in section 2.2.
